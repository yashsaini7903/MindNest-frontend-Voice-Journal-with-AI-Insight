...
import { MyThoughtAction } from "../store/store";

const MyThoughts = ()=>{
  const { MyThoughts } = useSelector((store) => store.MyThoughts);
  ...
  useEffect(() => {
    ...
    const fetchThoughts = async () => {
      try {
        ...
        const res = await fetch("http://localhost:3000/api/journal/user", {
          signal,
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="d-flex flex-wrap justify-content-center gap-3">
        {MyThoughtAction.length > 0 ? (
          MyThoughtAction.map((thought, index) => (
            <Card
              key={index}
              title={thought.title}
